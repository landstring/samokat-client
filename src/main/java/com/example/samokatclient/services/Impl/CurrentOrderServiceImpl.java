...
public class CurrentOrderServiceImpl implements CurrentOrderService {
    ...
    public String createCurrentOrder(String sessionToken) {
        ...
        NewOrderDto newOrderDto = createNewOrderDto(session, orderToken, paymentCode, orderDateTime);
        sendOrderMessage(newOrderDto);
        clearCart(session);
        return paymentCode;
    }

    @Override
    public CurrentOrderClientDto getCurrentOrderById(String sessionToken, String orderId) {
        Session session = getSession(sessionToken);
        ...
        return currentOrderClientMapper.toDto(currentOrderClient);
    }

    public void cancelCurrentOrder(String sessionToken, String orderId) {
        Session session = getSession(sessionToken);
        CurrentOrderClient currentOrderClient = currentOrderClientRepository.findById(orderId).orElseThrow(
                () -> new CurrentOrderNotFoundException("Текущий заказ не найден")
        );
        ...
        currentOrderClientRepository.delete(orderId);
    ...
    private void sendOrderMessage(NewOrderDto newOrderDto) {
        log.info("Отправлено сообщение в Kafka в топик {}. Сообщение: {}", "newOrder", newOrderDto);
        orderKafkaTemplate.send("newOrder", newOrderDto);
    }

    private void sendStatusMessage(NewStatusDto newStatusDto) {
        log.info("Отправлено сообщение в Kafka в топик {}. Сообщение: {}", "newStatusClient", newStatusDto);
        statusKafkaTemplate.send("newStatusClient", newStatusDto);
    }
